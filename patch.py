...
def patch_codec_commander(kext_path):
    ...
    with open(info_path, 'rb') as fp:
        plist = plistlib.readPlist(fp) if sys.version_info[0] < 3 else plistlib.load(fp)
        ...
        with open(info_path, 'wb') as fp:
            plistlib.writePlist(plist, fp) if sys.version_info[0] < 3 else plistlib.dump(plist, fp)
